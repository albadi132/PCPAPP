   ...
   <div
    ...
    >
      ...
      <div class="flex flex-col w-11/12 sm:w-5/6 lg:w-1/2 max-w-2xl mx-auto rounded-lg border border-gray-300 shadow-xl ">
        ...
        >
          <p class="text-2xl	 justify-center font-semibold text-gray-800">Creat Team</p>
          
        </div>
        <div class="flex flex-col px-6 py-5 bg-gray-50">
          <!-- Form Start -->
          <form method="POST" @submit.prevent="creatteam" @keydown="form.onKeydown($event)">   
            <AlertError :form="form" /> 
      
        <label class="text-gray-600 font-semibold text-lg">Choose a name for your team</label>
       <div
          class="input wrapper flex items-center"
        >
           <input  v-model="form.name" type="text" id="name" autocomplete="off" class="h-3 p-6 w-full border-2 border-gray-300 mb-5 rounded-md" placeholder="Team Name">
      ...
      <!-- Form End -->
        ...
        >
          ...
          <button @click="creatteam" :disabled="form.busy" class="px-4 py-2 text-white font-semibold bg-blue-500 rounded">
            Save
          </button>
        </div>
...
export default {
  ...
  data() {
    ...
    test() {
      ...
      const response = await this.form.post('/competition/'+ this.url +'/creatnewteam').then(({ data }) => { 
        if(data.status == 200)
