<template>
  <div>
    ...
    >
      ...
      <span class="uppercase text-sm font-semibold"> Add Question</span>
    </button>
    <!-- Start Modal -->
...
<div>
    ...
    <div class="relative inline-block w-full text-gray-700">
     ...
     v-model="form.problem" @change="onChange(form.problem)" >
        
        <option v-for="problem   in librarys"
    ...
    <div>
    <div class="relative inline-block w-full text-gray-700">
    <div class="text-1xl text-gray-500 ">Description: <p id="description" /> </div>
    <div class="text-1xl text-gray-500 ">Points: <p id="points" /></div>
    </div>
    </div>
...
export default {
  props: ["library" , "problems" ,"contest", "urlname"],
  name: "AddRemove",
  data() {
    return {
      librarys: JSON.parse(this.library),
  ...
  methods: {
    ...
    onChange(problem)
    {
      ...
      for( let i = 0 ; i < this.librarys.length ; i++)
            {
              if(problem == this.librarys[i].id)
              {
                document.getElementById('description').innerHTML =  this.librarys[i].description
                document.getElementById('points').innerHTML =  this.librarys[i].points

                for(let j = 0 ; j < this.allproblems.length ; j++)
                {
                  if(this.allproblems[j].id == this.librarys[i].id)
                  {
                     document.getElementById('existing').innerHTML =  'This question has already been in the competition'
                  }
    ...
    async addProblem() {
      const response = await this.form
        .post("/competition/" + this.url + "/mange/question/add")
        .then(({ data }) => {
          if (data.status == 200) {
