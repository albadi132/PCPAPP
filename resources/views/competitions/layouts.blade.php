...
<div class="flex justify-center">
  ...
  @endif
  <a href="{{route('scoreboard', ['name' => NameToUrl($contest->name) ])}}" class=" {{ 'competition/'.NameToUrl($contest->name).'/scoreboard' == request()->is('competition/'.NameToUrl($contest->name).'/scoreboard*') ? 'border-white text-white bg-green-400 ' : ' border-green-400 text-green-400 bg-white ' }} hover:bg-green-400 hover:text-white hover:border-white border-2  px-4 py-2 text-lg font-semibold tracking-wider  inline-flex items-center space-x-2 rounded ">
    <span>
      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
